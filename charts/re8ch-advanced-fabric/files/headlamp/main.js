(function(factory){if(typeof exports==='object'&&typeof module!=='undefined'){factory(require('react/jsx-runtime'),require('@kinvolk/headlamp-plugin/lib'),require('@kinvolk/headlamp-plugin/lib/CommonComponents'),require('@mui/material'));}else{factory(window.ReactJSXRuntime,window.Headlamp,window.Headlamp.CommonComponents,window.MaterialUI);}})(function(jsx,plugin,common,mui){'use strict';
  ...
  function View(){const result=plugin.K8s.ResourceClasses.ConfigMap.useList({namespace:'kube-system'});const items=(result.items||[]).filter(x=>((x.jsonData.metadata||{}).labels||{})[STATUS_LABEL]==='true').map(parse).sort((a,b)=>(a.node||'').localeCompare(b.node||''));
    const columns=[
    ...
    return jsx.jsxs(common.SectionBox,{title:'Advanced Fabric node network status',children:[jsx.jsx(mui.Alert,{severity:'info',sx:{mb:2},children:'Live host evidence. A failed node is repaired forward; it does not trigger a global fabric rollback.'}),jsx.jsx(common.Table,{data:items,columns}),items.map(r=>jsx.jsxs(mui.Accordion,{children:[jsx.jsx(mui.AccordionSummary,{children:jsx.jsx(mui.Typography,{children:`${r.node}: per-node ECMP view`})}),jsx.jsx(mui.AccordionDetails,{children:jsx.jsx('pre',{style:{whiteSpace:'pre-wrap',overflow:'auto'},children:JSON.stringify({ecmpRoutes:r.ecmpRoutes||[],peerRoutes:r.peerRoutes||[],pathRankings:r.pathRankings||{}},null,2)})})]},r.node))]});}
